const apiMethods = {
  methods: {
    ...
    resetuserData(data) {
      Lockr.set('userInfo', data.data.user)            // 用户信息
      Lockr.set('token', data.data.token)              // 权限认证
      window.axios.defaults.headers.Authorization = 'JWT ' + Lockr.get('token')
      // console.log(axios.defaults.headers.Authorization)
      setTimeout(() => {
        router.replace('/about_us/')
      }, 1000)
    },
    reAjax(url, data) {
      return new Promise((resolve, reject) => {
